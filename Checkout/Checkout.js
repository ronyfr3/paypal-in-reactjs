    ...
    if (paidBy && paidBy.length) {
      Axios({
        ...
        .then(function (response) {
          //handle success
          dispatch(createNotification(true, response.data.message));
          history.push("/");
        })
        .catch(function (response) {
          //handle error
          dispatch(createNotification(true, "something wrong!"));
        });
    }
  };
  return (
    <div>
      ...
      <button onClick={sendOrder} disabled={!check && !check.length}>
        Place order
      </button>
    </div>
  );
